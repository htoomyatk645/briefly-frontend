...
import { SEARCH_CLIP_METADATA, type SearchClipMetadata } from '../../data/searchClipMetadata'
...
function mergeFeedTiming(entry: SearchIndexEntry): SearchIndexEntry {
  const feed = FEED_QUEUE.find((ep) => ep.id === entry.feedEpisodeId)
  if (!feed) return entry
  return {
    ...entry,
    coverSrc: entry.coverSrc || feed.coverSrc,
